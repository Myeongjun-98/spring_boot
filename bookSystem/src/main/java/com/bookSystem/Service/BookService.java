...
public class BookService {
		...
		// 도서 클릭하여 내 장바구니에 넣기
		// 클릭한 도서의 bookid값 받아야되고, 현재 로그인한 회원 id(번호)가 필요
		...
		// 현재 로그인 한 회원의 장바구니 목록 띄우기 -> 컨트롤로부터 로그인 정보 가져오기
		public List<BookBasketDto> myBasketList(String email){
			...
			// 회원 id와 일치하는 모든 데이터 조회 ( mybasket테이블에서 )
			List<MyBasket> myBasketList = bookRepository.selectBasket(memberId);
			
			// 현재 로그인한 회원의 mybasket에 있는 도서번호(book_id)를 통해 도서정보 가져오기
		...
		// 장바구니에서 도서 클릭하여 내 대출목록에 넣기
			// 클릭한 도서의 bookid값 받아야되고, 현재 로그인한 회원 id(번호)가 필요
			public void myloanSave(int id, int bookId, String email) {
				...
				bookRepository.loanInsert(my);
			}
		
		// 반납메뉴 클릭하면 대여중인 도서목록 보이는 서비스 쪽 메서드
